...
void simple_thr(string path, string path_out, int begin_, int max_len, float thr_, int morph_measure_, int show_, CvMat* pMask)
{
...
void adapted_thresh(string path, string path_out, int begin_, int max_len, float thr_, int morph_measure_, int show_, CvMat* pMask, float sigma, float power_)// adapted thresholding
{
...
void global_temporal_mediane(string path, string path_out, int begin_, int max_len, float thr_, int morph_measure_, int show_, CvMat* pMask, int cannalYUV, int step, float sigma, float power_, float thr_milk, int areaSize)// mediane via time
{
	string lean,lean_mask,lean_clust;
	int show0= show_ ;
	alef00 al_;
...
void local_spacial_mediane(string path, string path_out, int begin_, int max_len, float thr_, int morph_measure_, int show_, CvMat* pMask)// mediane local
{
	string lean,lean_mask;
	float med;
	int show0= show_ ;
...
void local_temporal_mediane(string path, string path_out, int max_len, float thr_, int show_, CvMat* pMask, int medSize, int cannalYUV)
{
	string lean,lean_mask,lean_clust;
	int show0=show_;
	alef00 al_;
...
void shape_sceleton_find00(string path, string path_out, int begin_, int max_len, float thr_, int morph_measure_, int show_, CvMat* pMask, int minSizeBlob, int maxSizeBlob)
{
	string lean,lean_mask,lean_clust;
	 float med;
	 int show0=show_;
...
int main( int argc, char** argv )
{
	...
	// make mask
	IplImage* img = cvLoadImage("mask.jpg");//mask where we are working
	...
	if(routine_ == 1) simple_thr(path, path_out, begin_, max_len, thr_, morph_measure_, show_, mask);
	if(routine_ == 2) adapted_thresh(path, path_out, begin_, max_len, thr_, morph_measure_, show_, mask, 50.4, 3);
    if(routine_ == 4) global_temporal_mediane(path, path_out, begin_, max_len, thr_, morph_measure_, show_, mask, 2, 1, 50.4, 3, 0.5, 5);
	if(routine_ == 5) local_spacial_mediane(path, path_out, begin_, max_len, thr_, morph_measure_, show_, mask);
	if(routine_ == 6) local_temporal_mediane(path, path_out, max_len, thr_, show_, mask, 5, 2);
	if(routine_ == 7) shape_sceleton_find00(path, path_out, begin_, max_len, thr_, morph_measure_, show_, mask, 20, 20000);
