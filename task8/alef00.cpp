...
///////////////////////////////////////////////////
bool alef00::polar_feature(CvMat *SOURSE,CvPoint &centers,CvMat *POLAR,CvMat *feat_v)
{
	CvMoments moments;
	double M00, M01, M10;
	float sc,coll,n_;

	cvMoments(SOURSE,&moments,1);

	M00 = cvGetSpatialMoment(&moments,0,0);
	if(M00>0){
		M10 = cvGetSpatialMoment(&moments,1,0);
		M01 = cvGetSpatialMoment(&moments,0,1);
		centers.x = (int)(M10/M00);
		centers.y = (int)(M01/M00);


		cvLogPolar( SOURSE, POLAR, cvPoint2D32f(centers.x,centers.y), 30, CV_INTER_LINEAR+CV_WARP_FILL_OUTLIERS  );
                                            
		for(int wwl =0;wwl<POLAR->rows;wwl++){
			coll=0;
			for(int qql =0;qql< POLAR->cols ;qql++){
				if(CV_MAT_ELEM( *POLAR, BYTE, wwl, qql)>0){
					//sc=exp((float)qql)/aa;
					sc=qql^2 ;
					coll=coll+sc;
				}
			}//for(qq =0;qq<w_2;qq++)
			CV_MAT_ELEM( *feat_v, float, 0, wwl)=coll;
		}//for(ww =0;ww<w_2;ww++)

		n_=cvNorm(feat_v, NULL, CV_L2);
		cvScale(feat_v,feat_v,1/n_);
...
void alef00::visu_string_float(CvMat* vect,int n_, std::string name,int scale_x)
{
...
float *elem_=(float*)(vect->data.fl);
int x_=    vect->cols;
float min_=*std::min_element(elem_, elem_+x_);
float max_=*std::max_element(elem_, elem_+x_);
float diam = max_-min_;
...
float yt,yt1;
int curr_,prew_;
for(ww =1;ww<vect->cols;ww++)
{
yt=CV_MAT_ELEM( *vect, float, 0, ww);
 yt1=CV_MAT_ELEM( *vect, float, 0, ww-1);
yt=50.001*(yt-min_)/diam;yt1=50.001*(yt1-min_)/diam;

curr_=250-(int)yt;
prew_=250-(int)yt1;

cvLine( VISU,cvPoint(10+scale_x*(ww-1),10+prew_),cvPoint(10+scale_x*(ww),10+curr_),CV_RGB(255,100,30), 0, 0, 0 );
}//for(ww =0;ww<qwe;ww++)       
cvShowImage(name.data(), VISU );
cvWaitKey();
cvReleaseMat(&VISU);       
